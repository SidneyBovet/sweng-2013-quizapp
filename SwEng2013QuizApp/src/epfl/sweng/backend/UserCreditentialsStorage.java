...
public final class UserCreditentialsStorage {
	...
	private UserCreditentialsStorage(Context context) {
		 ...
		 *  The bit MODE_MULTI_PROCESS can also be used if multiple processes 
		 *  are mutating the same SharedPreferences file. MODE_MULTI_PROCESS is
		 *   always on in apps targetting Gingerbread (Android 2.3) and below,
	 ...
	 * @param sessionID that the user received from Tequila
	 */
	public void takeAuthentification(int sessionID) {
		editor.putInt(keySessionIDName, sessionID);
		/* Once you have changed the value you have to call the apply() method to 
		apply your asynchronously to the file system. The usage of the commit()
		method is discouraged, as it write the changes synchronously 
		to the file system.*/
		editor.commit();
	}
	 ...
	 * @param sessionID that is checked to be in the table.
	 ...
	 * @param sessionID that we want to remove because the user logged out
	 */
