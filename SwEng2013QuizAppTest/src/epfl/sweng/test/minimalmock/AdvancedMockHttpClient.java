...
public class AdvancedMockHttpClient extends DefaultHttpClient {
	...
	private final Set<CannedResponse> responsesToUseOnlyOnce = new HashSet<CannedResponse>();
	...
	public void pushCannedResponse(String requestRegex, int status,
			String responseBody, String contentType) {
		responses.add(0, new CannedResponse(Pattern.compile(requestRegex),
				status, responseBody, contentType));
	 ...
	 * 
	 * @param requestRegex
	 *            The regular expression used for matching a request to this
	 *            response
	 * @param status
	 ...
	 * @param onlyOnce
	 *            Allows to specify whether this canned response should be used
	 *            once and then be removed (single-use response)
	 */
	public void pushCannedResponse(String requestRegex, int status,
			String responseBody, String contentType, boolean onlyOnce) {
		CannedResponse cannedResponse = new CannedResponse(
				Pattern.compile(requestRegex), status, responseBody,
				contentType);
	...
	public HttpResponse processRequest(HttpRequest request) {
		...
		for (CannedResponse cr : responses) {
			if (cr.pattern.matcher(request.getRequestLine().toString()).find()) {
				Log.v("HTTP", "Mocking request since it matches pattern "
						+ cr.pattern);
				...
				return new AdvancedMockHttpResponse(cr.statusCode,
						cr.responseBody, cr.contentType);
			}
		}
	...
	private static String convertStreamToString(InputStream is, String ecoding)
			throws IOException {
...
class AdvancedMockHttpResponse extends BasicHttpResponse {
	public AdvancedMockHttpResponse(int statusCode, String responseBody,
			String contentType) {
		super(new ProtocolVersion("HTTP", 1, 1), statusCode,
		...
		if (responseBody != null) {
			try {
				...
				Log.e(this.getClass().getName(), "setEntity(): Entity "
